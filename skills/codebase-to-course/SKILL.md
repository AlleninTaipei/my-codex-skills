...
Each agent writes its module file(s) to `course-name/modules/`. Short modules (3 screens) can be paired — two briefs given to one agent.
...
- **`references/interactive-elements.md`** — Implementation patterns for every interactive element: code↔English translations, group chat animations, message flow visualizations, architecture diagrams, pattern cards, callout boxes. Read the relevant sections during Phase 3.
